...
학술·개발 도메인 문서에는 **개인정보(이름/이메일/연락처 등)** + **자격 증명(API 키·토큰, 내부 엔드포인트)** + **업무상 민감 정보(미발표 연구 데이터, 내부 설계 결정)**가 혼재할 수 있다. 업무상 민감 정보는 챗봇의 핵심 컨텐츠이므로 일률 마스킹 불가. → **계층화 + 어드민 판단** 모델 채택.
...
- 한국 패턴 정규식으로 검출:
  ...
  - API 키·토큰 (`sk-`, `ghp_`, `AKIA` 등 접두사 패턴)
- 비용 무료, 지연 무시할 수준
...
- 검출 결과 표시 → 어드민이 선택:
  ...
  - **(b) 그대로 업로드** — 업무상 민감 정보가 챗봇 핵심인 경우
  - **(c) 업로드 취소**
...
6. **PII 마스킹 false positive**: 정규식 오검출 (API 키 형식과 다른 임의 문자열 등) — 시드 PDF로 정확도 측정 후 패턴 튜닝.
